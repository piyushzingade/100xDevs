...
export const ModernStack = () => {
    ...
    return (
        <div className="grid grid-cols-4 gap-2 md:gap-3 w-full max-w-[360px]">
            {techStack.map((tech, idx) => (
                <motion.div
                    key={idx}
                    className="flex flex-col items-center gap-1 md:gap-1.5 cursor-pointer group/tech"
                    whileTap={{ scale: 0.95 }}
                ...
                >
                    ...
                    <span className="text-[7px] md:text-[8px] font-medium text-gray-500 group-hover/tech:text-gray-300 transition-colors text-center">
                        {tech.label}
                    </span>
                </motion.div>
            ))}
